 ...
 * each with its own label, expiry and revocation state. The link's secret token
 * is NEVER stored; only its sha256 hash is persisted (mirrors the org-invitation
 * pattern). Tenant isolation for the public read path is anchored entirely on the
 * `ownerId` recorded here — the client never supplies it.
 */
...
export interface IDashboardShare extends Document {
  ...
  tokenHash: string;          // sha256(token) — raw token is returned once and never stored.
